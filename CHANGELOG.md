...
- Phase 11 human-readable comparison summaries: `--format markdown` and `--format github-markdown` render escaped count/finding summaries, `--summary-only` provides compact CI output, and a read-only GitHub Actions example preserves/uploads the summary before enforcing `--fail-on-new`.
